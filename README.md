...
## Вывод
Мы выделили оптимальные параматеры для каждой политики изменения темпа обучения. Хоть и разница в точности между нейронными сетями из каждой политики небольшая, можно заметить, что с политикой cosine decay with restarts нейронная сеть довольна быстрее сошлась к своему пиковому состоянию (на 14 эпохе). Однако, думаю, можно добиться более хороших результатов, если еще проверить другие значения параметров.
